...
function Artists() {
    ...
    const { data: artists } = useQuery({
        queryKey: ['artists', { name: searchName, user_role_tags: roleTags }],
        queryFn: () => fetchArtists({ name: searchName, user_role_tags: roleTags }),
                ...
                <div className="Artist-page-cards">
                    ...
                    {(artists?.map((artist, index) => (
                                <GigCard 
                                    ...
                                    imageUrl={artist.profile_image?.public_url || ''}
                                    title={artist.name} 
                                    bio={artist.bio} 
                                    tags={artist.user_genre_tags.concat(artist.user_role_tags)} 
                                    buttonText="View Profile"
                                    onButtonClick={() => console.log('Artist Clicked', artist.name)}
                                />
                            ))
                        )}
                </div>
